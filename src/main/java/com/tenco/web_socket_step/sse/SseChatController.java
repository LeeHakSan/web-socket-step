...
public class SseChatController {
    ...
    // produces : 서버측에서 이러한 데이터를 생산 한다.
    // 일반적인 코드를 작성할 때 기본적으로 다 생략 했었음
    // produces = MediaType.TEXT_HTML_VALUE - HTML 파일 형식 (뷰리졸브)
    // produces = MediaType.APPLICATION_JASON_VALUE

    // HTTP/1.1 OK
    // Content-Type: text/event-stream

    // 2. 클라이언트가 이 주소를 구독함 (sse 구독 이벤트 처리)
    @GetMapping(value = "/sse/connect", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    @ResponseBody // 뷰가 아니라 객체를 반환해야함
    public SseEmitter connect() {
        // clientId 는 서버측에서만 관리하는 값이다
