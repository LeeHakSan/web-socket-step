...
public class SseChatService {
    ...
    // 접속한 클라이언트들을 보관할 자료구조 - ConcurrentHashMap 멀티스레드에서 안전함
    private final Map<String, SseEmitter> emitterMap = new ConcurrentHashMap<>();

    // 1. 클라이언트가 접속 하면 호출됨(SSE 연결)
    public SseEmitter createConnection(String clientId) {
        // 파이프 라인 연결 - 유지시간이 60초 권장
        SseEmitter emitter = new SseEmitter(60 * 1000L);
        // 보관
        emitterMap.put(clientId, emitter);

        // 필수 콜백 함수 등록
        // 연결 종료 / 타임 아웃시 Map 목록에서 제거 처리 (불필요한 메모리 누수 방지)
        ...
        // 1000L : 서버와 클라이언트가 연결(파이프)을 유지하는 최대시간 (현재 1초)
        // 1초 뒤에 : 서버가 클라이언트 연결을 끊어버림
        // 참고 : 0L 로 설정하면 무한으로 계속 연결이 되지만 실무에선 좀비 커넥션이 생성 돼서
        // 실제로 0L 을 하지 않는다. - 폴링 방식이 절대 아님
        try {
    ...
    private void broadcast(String message, String sender) {
        // 프로토콜 {json 형식}, 보낸사람 : 메세지
        String formattedMessage = sender + ": " + message; // 반드시 공백 한 칸
